 ...
 * @package WooCommerceTicketManager\Lib
...
abstract class Plugin implements PluginInterface {
	...
	public function load_notices() {
		$notice = isset( $_GET['notice'] ) ? sanitize_key( $_GET['notice'] ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		if ( $this->has_premium() && ! $this->is_premium_active() ) {
			...
			$pro_link        = add_query_arg(
				array(
					...
					'utm_term'     => 'go-pro',
					'utm_id'       => $this->get_slug(),
				),
				$this->get_premium_url()
			);
			$links['go_pro'] = sprintf( '<a href="%1$s" target="_blank" style="color: #39b54a; font-weight: bold;">%2$s</a>', esc_url( $pro_link ), esc_html__( 'Go Pro', 'wc-ticket-manager' ) );
		}
	...
	*/
	...
	public function get_meta_links() {
		...
		if ( ! empty( $this->get_docs_url() ) ) {
			$links['docs'] = array(
				'label' => __( 'Documentation', 'wc-ticket-manager' ),
		...
		if ( ! empty( $this->get_support_url() ) ) {
			$links['support'] = array(
				'label' => __( 'Support', 'wc-ticket-manager' ),
		...
		if ( ! empty( $this->get_review_url() ) ) {
			$links['review'] = array(
				'label' => __( 'Review', 'wc-ticket-manager' ),
				'url'   => $this->get_review_url(),
			);
		}

		$links['plugins'] = array(
			'label' => __( 'More Plugins', 'wc-ticket-manager' ),
	...
	public function get_action_links() {
		...
		if ( ! empty( $this->get_settings_url() ) ) {
			$links['settings'] = array(
				'label' => __( 'Settings', 'wc-ticket-manager' ),
