 ...
 * @since 1.0.0
 * @version 1.0.7
 * @subpackage WooCommerceTicketManager\Lib\Settings
...
abstract class Settings {
	...
	public function save_settings() {
		...
		if ( class_exists( '\WC_Admin_Settings' ) && ! empty( $settings ) && \WC_Admin_Settings::save_fields( $settings ) ) {
			add_settings_error( $class_name, 'response', __( 'Settings saved.', 'wc-ticket-manager' ), 'updated' );
			...
			foreach ( $promo_plugins as $promo_plugin ) {
				$promo_plugin = wp_parse_args(
					...
					array(
						...
						'basename'    => '',
						'slug'        => '',
						'badge'       => esc_html__( 'Recommended', 'wc-ticket-manager' ),
						'button'      => esc_html__( 'Install Now', 'wc-ticket-manager' ),
						'installed'   => false,
					)
				);
	...
	protected function output_support_widget() {
		...
		if ( ! empty( $support_links ) ) {
			?>
			<div class="pev-panel">
				<h3><?php esc_html_e( 'Need Help?', 'wc-ticket-manager' ); ?></h3>
				<ul>
	...
	public function get_support_links() {
		return array(
			'facebook'        => array(
				'label' => __( 'Join our Community', 'wc-ticket-manager' ),
				'url'   => 'https://www.facebook.com/groups/pluginever',
			),
			'feature-request' => array(
				'label' => __( 'Request a Feature', 'wc-ticket-manager' ),
				'url'   => 'https://www.pluginever.com/contact/',
			),
			'bug-report'      => array(
				'label' => __( 'Report a Bug', 'wc-ticket-manager' ),
				'url'   => 'https://www.pluginever.com/contact/',
			),
		);
	}
